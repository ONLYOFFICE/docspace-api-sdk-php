# onlyoffice/docspace-api-sdk.OpenAPI\Client\Api.AccessToDevToolsApi
...
```php
getTenantAccessDevToolsSettings(): \OpenAPI\Client\Model\TenantDevToolsAccessSettingsWrapper
```
...
try {
    $result = $apiInstance->getTenantAccessDevToolsSettings();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AccessToDevToolsApi->getTenantAccessDevToolsSettings: ', $e->getMessage(), PHP_EOL;
}
```
