# onlyoffice/docspace-api-sdk.OpenAPI\Client\Api.PasswordApi
...
```php
sendUserPassword($email_member_request_dto): \OpenAPI\Client\Model\StringWrapper
```
...
try {
    $result = $apiInstance->sendUserPassword($email_member_request_dto);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PasswordApi->sendUserPassword: ', $e->getMessage(), PHP_EOL;
}
```
