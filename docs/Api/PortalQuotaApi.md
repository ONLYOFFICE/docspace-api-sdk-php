...
```php
getUpcomingPayments($refresh): \OpenAPI\Client\Model\UpcomingPaymentArrayWrapper
```
...
[**\OpenAPI\Client\Model\UpcomingPaymentArrayWrapper**](../Model/UpcomingPaymentArrayWrapper.md)
...
try {
    $result = $apiInstance->getUpcomingPayments($refresh);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling QuotaApi->getUpcomingPayments: ', $e->getMessage(), PHP_EOL;
}
```
