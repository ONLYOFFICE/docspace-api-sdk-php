# # CreateThirdPartyRoom

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**create_as_new_folder** | **bool** | Specifies whether to create a third-party room as a new folder or not. | [optional]
**title** | **string** | The third-party room name to be created. |
**room_type** | [**\OpenAPI\Client\Model\RoomType**](RoomType.md) | The third-party room type to be created. |
**private** | **bool** | Specifies whether to create the private third-party room or not. | [optional]
**indexing** | **bool** | Specifies whether to create the third-party room with indexing. | [optional]
**deny_download** | **bool** | Specifies whether to deny downloads from the third-party room. | [optional]
**color** | **string** | The color of the third-party room. | [optional]
**cover** | **string** | The cover of the third-party room. | [optional]
**tags** | **string[]** | The list of tags of the third-party room. | [optional]
**logo** | [**\OpenAPI\Client\Model\LogoRequest**](LogoRequest.md) | The logo request parameters of the third-party room. | [optional]

[[Back to Model list]](../../README.md#models) [[Back to API list]](../../README.md#endpoints) [[Back to README]](../../README.md)
